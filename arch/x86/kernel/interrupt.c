#include "kernel.h"
#include "btype.h"
#include "x86/interrupt/interrupt64.h"
#include "interrupt.h"
#include "memory.h"
#include "shell.h"
...
__attribute__((interrupt))
void ErrorInterrupt(interrupt_frame frame)
{
	return;
}

__attribute__((interrupt))
void PageErrorInterrupt(interrupt_frame frame)
{
	for (;;)
	{
		asm("hlt\n\t");
...
{
	IDTR idtr = (IDTR){
		.address = (UINT64)allocA4KBPage(MAP_TYPE_OSDATA, MAP_FLAG_READ | MAP_FLAG_WRITE),
		.size = 4095
	};
	setIDTR(idtr);
	for (int i = 0; i < 32; ++i)
	{
		setInterrupt(getCS(), ErrorInterrupt, 0, DP | DPL0 | TrapGate, i);
	}
	for (int i = 32; i < 256; ++i)
	{
		setInterrupt(getCS(), CommonInterrupt, 0, DP | DPL0 | InterruptGate, i);
	}
	setInterrupt(getCS(), PageErrorInterrupt, 0, DP | DPL0 | TrapGate, 14);
